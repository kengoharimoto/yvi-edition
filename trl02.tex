\section{Commentary on Pātañjalayogaśāstra 1.2}
\bht{However,\pratikap{SUTR2INTRO} when all the activities have ceased, [\samadhi] is \asamprajnata. Accordingly}—the word ``however (\emph{tu})'' is used in the sense of delimiting—\bht{the next \sutra\ was produced (\emph{pravavṛte})}, was issued (\emph{pravṛttam}), \bht{with the intention of denoting the characteristic}\footnote{[T] I have emended, as did the editors of the 1952 edition, the reading \emph{tasyaivaṃviṣayasya} found in all the manuscripts to \emph{tasyaivaṃvidhasya}. While glossing \emph{tasya}, a pronoun, with \emph{evaṃvidhasya} is natural, doing so with \emph{evaṃviṣayasya} is unintelligible, especially the subject is \emph{asamprajñāta-samādhi} where one does not expect it to have objects. This was most likely an error caused by mental expectations of a scribe who saw \emph{evaṃvi°}.} \bht{of that}—of such type of—\samadhi, i.e., of only \asamprajnata\ alone.\footnote{[T] The repetition of \emph{eva} and \emph{keavlasya} might make one to suspect the second word (\emph{kevalasya}) was a gloss for the first (\emph{eva}) and hence the word \emph{eva} was in the root text our author was commenting upon. However, the repetition and \emph{eva} and \emph{kevala} is relatively common especially in the works ascribed to \Sankara, whose works the \YVi\ resembles much stylistically.} 

\begin{quotation}
\st{Yoga
\pratikap{SUTRA2} is cessation of activities of mind.~(\rYS{1.02}{1.2})}
\end{quotation}

[Objection] Is it not that this \sutra\ was issued in order to denote the characterization of it (yoga)? That being the case, [the \sutra] should say ``Yoga has cessation of activities of mind [as the characterization].'' [That the to-be-characterized and the characterization] sharing the same syntactical position (\samanadhikaranya) is not appropriate. For, the characterization should not be the same as the to-be-characterized. Or, the \sutra\ should say ``Yoga is characterized by cessation of activities of mind.''\footnote{It would seem a little strange that this criticism is raised. I do not find anyone who claims that \laksya\ and \laksana\ must not be in the \samanadhikaranya\ relationsihp. One place that might be relevant is the commentary by \Sabara\ on \rJS{1.1.2} \emph{codanālakṣaṇo 'rtho dharmaḥ}. \Sabara\ says \emph{lakṣyate yena tal lakṣaṇam| dhūmo lakṣaṇam agner iti hi vadanti}. Note the similarity to the expressions of the \sutra\ and the example. }
% Does this discussion have anything to do with Nāgārjuna's MMK discussion on lakṣya and lakṣaṇa? Abhisamayālaṅkāravṛtti? 
...
``[It (\emph{asamprajñāta-samādhi}) is] yoga [and it is] the termination of the activities of the mind (\rYS{1.02}{1.2}).''\footnote{\label{YS12first}The translation here follows the interpretation of the \sutra\ in the YVi where the author states that the \sutra\ is the characteristic of the \emph{asamprajñāta-samādhi}.  This is somewhat counter-intuitive, but our author is clear about \sutra\ 1.2 being the characteristic of \emph{asamprajñāta-samādhi}.  See, for example, the last two paragraphs of the commentary of the current \sutra.  See also note \ref{no_yogah}.}
